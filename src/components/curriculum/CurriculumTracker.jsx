    ...
    const toggleTopic = (topicId) => {
        ...
        const updated = curriculum.map(t => {
            if(t.id === topicId) {
                ...
                // Ana konu tiklenirse, altındaki tüm konular da tiklensin
                return { 
                    ...t, 
                        ...
                        {curriculum.map((topic) => (
                            <div key={topic.id} className="flex flex-col group/topic">
                                ...
                                <div className="flex items-start gap-4">
                                    <button onClick={() => toggleTopic(topic.id)} className={`mt-1 flex-shrink-0 transition-colors ${topic.isCompleted ? 'text-indigo-500' : 'text-slate-400 hover:text-indigo-400'} ${!isTeacherMode && 'cursor-default pointer-events-none'}`}>
                                ...
                                <div className="pl-11 mt-3 space-y-3">
                                    {topic.subTopics?.map(sub => (
                                        <div key={sub.id} className="flex items-center gap-3 group/sub">
                                            <button onClick={() => toggleSubTopic(topic.id, sub.id)} className={`flex-shrink-0 transition-colors ${sub.isCompleted ? 'text-indigo-500' : 'text-slate-400 hover:text-indigo-400'} ${!isTeacherMode && 'cursor-default pointer-events-none'}`}>
