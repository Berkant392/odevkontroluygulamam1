            ...
            <div className="max-w-4xl mx-auto bg-white rounded-3xl shadow-float border border-slate-100 flex flex-col overflow-hidden h-[80vh]">
                <div className="p-5 md:p-6 border-b border-slate-100 flex items-center bg-slate-50 gap-4">
                    <div className="bg-purple-100 p-3 rounded-xl">
                        <Library size={24} className="text-brandPurple"/>
                    </div> 
                    <div>
                ...
                <div className="p-4 bg-white border-b border-slate-100">
                    <div className="flex bg-slate-100 p-1.5 rounded-xl">
                        <button onClick={() => setLibraryCategory(LIBRARY_TYPES.TOPIC)} className={`flex-1 py-3 rounded-xl text-xs md:text-sm font-bold transition-all hover-lift ${libraryCategory === LIBRARY_TYPES.TOPIC ? 'bg-white text-brandPurple shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Ödevler</button>
                ...
                <div className="p-5 bg-slate-50 border-b border-slate-100 flex flex-col md:flex-row gap-3 items-center">
                    <input 
                        type="text" 
                        className="flex-1 w-full p-4 bg-white border border-slate-200 rounded-2xl text-sm font-bold text-slate-800 focus:border-brandPurple outline-none shadow-sm transition-colors" 
                    ...
                    <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => { addLibraryItem(libraryInput); setLibraryInput(''); }} className="bg-brandPurple text-white px-8 py-4 rounded-2xl shadow-glow font-black w-full md:w-auto hover:bg-purple-700 transition-colors">
                        Ekle
                    </motion.button>
                </div>
                        ...
                        <div key={item.id} className="flex justify-between items-start p-5 bg-white border border-slate-100 shadow-sm rounded-2xl group hover:border-purple-200 hover:shadow-md transition-all">
                            <div className="flex flex-col">
                                ...
                                {item.type === LIBRARY_TYPES.CURRICULUM && item.subTopics && item.subTopics.length > 0 && (
                                    <div className="flex flex-col gap-1.5 mt-3 pl-4 border-l-2 border-brandPurple/30">
