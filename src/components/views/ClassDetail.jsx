...
const ClassDetail = ({ 
    ...
    newStudentPhone, setNewStudentPhone, addStudent, updateGrade, openCellNoteModal, setModalData, setModalInputVal, 
    setModalDateVal, setModalPdfVal, setModalPhoneVal, setModalType, deleteStudent, handlePrintStudentReport, 
    ...
    const handleEditStudentClick = (student) => {
        ...
        if(setModalPhoneVal) setModalPhoneVal(student.phone || "");
        ...
        <motion.div key="class-detail" initial={{ opacity: 0, y: 30, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ type: "spring", stiffness: 260, damping: 20 }} className="bg-white rounded-[2rem] shadow-float border border-slate-200 overflow-hidden relative z-10">
            ...
            <div className={`p-4 md:p-6 border-b flex flex-col md:flex-row justify-between items-start md:items-center gap-3 md:gap-4 ${selectedClass.type === 'vip' ? 'bg-gradient-to-r from-yellow-50 to-white border-yellow-100' : 'bg-gradient-to-r from-slate-50 to-white border-slate-100'}`}>
                <div className="flex items-center gap-3 md:gap-4 w-full md:w-auto">
                    <div className={`p-2.5 md:p-3 rounded-xl shadow-inner ${selectedClass.type === 'vip' ? 'bg-yellow-100 text-amber-600' : 'bg-purple-100 text-primary'}`}><Layout size={20}/></div>
                    <div>
                ...
                <div className="flex flex-wrap items-center gap-1.5 w-full md:w-auto justify-end">
                    <div className="flex items-center gap-1.5 bg-white px-2.5 py-1.5 rounded-xl border border-slate-200 shadow-sm mr-1">
                        <div className={`w-6 h-6 md:w-8 md:h-8 rounded-full border-2 md:border-4 ${selectedClass.type === 'vip' ? 'border-yellow-200' : 'border-purple-100'} flex items-center justify-center relative`}>
                            <svg className="w-full h-full transform -rotate-90 absolute" viewBox="0 0 36 36"><path className={selectedClass.type === 'vip' ? "text-amber-500" : "text-primary"} strokeDasharray={`${calculateStats(selectedClass.students, selectedClass.topics).percentage}, 100`} d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" strokeWidth="4" /></svg>
                        </div>
                    ...
                    {isMobile ? (
                        <div className="space-y-3.5">
                            ...
                            <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-float mb-4">
                                <h4 className="font-black text-slate-800 mb-3 text-xs flex items-center gap-1.5 uppercase tracking-widest"><BookOpen size={15} className="text-primary"/> Mobil Ödev Yönetimi</h4>
                                <div className="space-y-3">
                                    {reversedTopics.map(topic => (
                                        ...
                                        <div key={topic.id} className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                                            <div className="flex justify-between items-center mb-2 pb-2 border-b border-slate-200"><span className="font-black text-slate-700 text-xs uppercase tracking-wide truncate pr-2">{getSafeText(topic.title)}</span><div className="flex gap-1 shrink-0"><button title="Düzenle" onClick={() => { setModalData({ classId: selectedClass.id, topicId: topic.id, currentTitle: topic.title }); setModalInputVal(topic.title); setModalDateVal(topic.date || ''); setModalType('edit-topic'); }} className="p-1.5 bg-white border border-slate-200 text-slate-500 hover:text-primary rounded-lg shadow-sm transition-colors"><Pencil size={12}/></button><button title="Kaynak Ekle" onClick={() => { setModalData({ classId: selectedClass.id, topicId: topic.id }); setModalType('source'); }} className="px-2 py-1.5 bg-primary text-white rounded-lg shadow-glow flex items-center gap-1 text-[10px] font-black tracking-wider transition-colors"><Plus size={10}/> KAYNAK</button></div></div>
                                            <div className="flex flex-col gap-1.5">
                                                {topic.subColumns?.map(col => (
                                                    // 🔥 GÜNCELLEME: Kaynak listesi dolgusu p-3'ten p-2.5'e sıkıştırıldı
                                                    <div key={col.id} className="flex justify-between items-center text-[11px] text-slate-600 bg-white p-2.5 rounded-lg border border-slate-200 shadow-sm"><span className="font-bold truncate pr-2">{getSafeText(col.title)}</span><div className="flex gap-0.5 shrink-0"><button title="Düzenle" onClick={() => { setModalData({ classId: selectedClass.id, topicId: topic.id, colId: col.id, currentTitle: col.title }); setModalInputVal(col.title); setModalPdfVal(col.pdfLink || ""); setModalType('edit-source'); }} className="p-1.5 bg-slate-50 text-slate-500 hover:text-primary rounded-md transition-colors"><Pencil size={11}/></button><button title="Sil" onClick={() => deleteColumn(selectedClass.id, topic.id, col.id)} className="p-1.5 bg-rose-50 text-rose-500 hover:bg-rose-100 rounded-md transition-colors"><Trash2 size={11}/></button></div></div>
                                                ))}
                            ...
                            <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex flex-col gap-1.5">
                                <input type="text" placeholder="Yeni Öğrenci Adı..." className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-700 w-full focus:border-primary outline-none font-medium" value={newStudentName} onChange={(e) => setNewStudentName(e.target.value)} onKeyDown={(e) => { if(e.key === 'Enter') addStudent(selectedClass.id); }} />
                                <div className="flex gap-1.5">
                                    <input type="text" placeholder="Tel (Opsiyonel)" className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-700 w-full focus:border-primary outline-none font-medium" value={newStudentPhone || ''} onChange={(e) => {if(setNewStudentPhone) setNewStudentPhone(e.target.value);}} onKeyDown={(e) => { if(e.key === 'Enter') addStudent(selectedClass.id); }} />
                                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => addStudent(selectedClass.id)} className={`text-white px-3.5 rounded-xl text-xs font-black shadow-md ${selectedClass.type === 'vip' ? 'real-gold-bg text-slate-900' : 'bg-primary'}`}>EKLE</motion.button>
                                </div>
                            </div>
                        ...
                        <div className="table-container">
                            <table className="w-full text-left border-collapse">
                                        ...
                                        {reversedTopics.map((topic, i) => {
                                            ...
                                            return ( 
                                                <React.Fragment key={topic.id}>
                                                    ...
                                                    {topic.subColumns?.map(col => ( 
                                                        <th key={col.id} className={`p-3 border-b border-r border-slate-200 sticky-header-sub ${theme.sub} min-w-[150px] align-top`}><div className="flex flex-col items-center justify-between h-full min-h-[50px]"><span className="font-bold text-xs text-slate-700 whitespace-normal text-center leading-tight mb-2 break-words max-w-[140px]">{getSafeText(col.title)}</span><div className="flex items-center gap-1 shrink-0">{col.pdfLink && <PdfDownloadButton link={col.pdfLink} isTeacher={true} />}<button title="Ayarlar" onClick={(e) => { e.stopPropagation(); setActiveColMenu({ classId: selectedClass.id, topicId: topic.id, colId: col.id, anchorEl: e.currentTarget }); }} className="text-slate-400 hover:text-primary bg-white/50 p-1.5 rounded-full shadow-sm transition-colors"><MoreVertical size={14}/></button></div></div></th> 
                                                    ))} 
                                                </React.Fragment> 
                                            );
                                        })}
                                ...
                                <tbody>
                                                ...
                                                <div className="flex justify-between items-center group">
                                                    <div className="flex flex-col gap-1 cursor-pointer" onClick={() => openStudent(std)}>
                                                        <div className="flex items-center gap-3">
                                                            <motion.div whileHover={{ scale: 1.1 }} className={`w-8 h-8 rounded-full ${selectedClass.type === 'vip' ? 'bg-yellow-100 text-amber-600' : 'bg-purple-100 text-primary'} flex items-center justify-center font-black text-xs`}>{getSafeText(std.name).charAt(0)}</motion.div>
                                                            <span className={`text-sm font-bold text-slate-700 group-hover:${selectedClass.type === 'vip' ? 'text-amber-600' : 'text-primary'} transition-colors`}>{getSafeText(std.name)}</span>
                                                            <button title="Düzenle" onClick={(e) => { e.stopPropagation(); handleEditStudentClick(std); }} className="text-slate-300 hover:text-primary opacity-0 group-hover:opacity-100 transition-opacity"><Pencil size={14}/></button>
                                                        </div>
                                                    </div>
                                    ...
                                    <tr>
                                        <td className="sticky-col-left p-4 border-r border-slate-200 border-t border-slate-200 bg-slate-50"><div className="flex flex-col gap-2"><div className="flex gap-2"><div className="w-8 h-8 rounded-full bg-slate-200 flex items-center justify-center text-slate-400 shrink-0"><UserPlus size={16}/></div><input type="text" placeholder="Yeni Öğrenci Ekle..." className="bg-white border border-slate-300 rounded-xl px-3 py-1.5 text-sm text-slate-700 w-full focus:border-primary outline-none font-medium shadow-sm" value={newStudentName} onChange={(e) => setNewStudentName(e.target.value)} onKeyDown={(e) => { if(e.key === 'Enter') addStudent(selectedClass.id); }} /></div><div className="flex gap-2 pl-10"><input type="text" placeholder="Telefon Numarası..." className="bg-white border border-slate-300 rounded-xl px-3 py-1.5 text-sm text-slate-700 w-full focus:border-primary outline-none font-medium shadow-sm" value={newStudentPhone || ''} onChange={(e) => {if(setNewStudentPhone) setNewStudentPhone(e.target.value);}} onKeyDown={(e) => { if(e.key === 'Enter') addStudent(selectedClass.id); }} /><motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => addStudent(selectedClass.id)} className={`text-white px-3 rounded-xl text-xs font-bold shadow-md transition-colors ${selectedClass.type === 'vip' ? 'real-gold-bg text-slate-900' : 'bg-primary'}`}>EKLE</motion.button></div></div></td>
