            ...
            <div className="max-w-4xl mx-auto bg-white rounded-3xl shadow-float border border-slate-100 overflow-hidden">
                <div className="flex border-b border-slate-100">
                    <button 
                        ...
                        className={`flex-1 py-4 font-black tracking-widest text-xs uppercase flex items-center justify-center gap-2 transition-colors ${notifTab === 'send' ? 'bg-brandPurple/5 text-brandPurple border-b-2 border-brandPurple' : 'text-slate-400 hover:bg-slate-50'}`}
                    >
                        <Send size={16} /> Yeni Gönder
                    </button>
                    <button 
                        onClick={() => setNotifTab('history')} 
                        className={`flex-1 py-4 font-black tracking-widest text-xs uppercase flex items-center justify-center gap-2 transition-colors ${notifTab === 'history' ? 'bg-brandPurple/5 text-brandPurple border-b-2 border-brandPurple' : 'text-slate-400 hover:bg-slate-50'}`}
                    >
                    ...
                    <div className="p-6 md:p-8 space-y-6">
                        <div className="flex flex-col gap-4 mb-6">
                            <input 
                                ...
                                className="w-full border-2 border-slate-200 rounded-xl p-3 font-bold text-slate-700 outline-none focus:border-brandPurple transition-colors"
                                value={notifTitle} 
                                onChange={e => setNotifTitle(e.target.value)}
                            />
                            <textarea 
                                placeholder="Bildirim İçeriği (Push bildirimi olarak da gidecektir)..." 
                                className="w-full border-2 border-slate-200 rounded-xl p-3 font-medium text-slate-700 outline-none focus:border-brandPurple h-28 resize-none transition-colors"
                                value={notifText} 
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
                                <div className="bg-slate-50 p-3 border-b border-slate-100 flex justify-between items-center">
                                    ...
                                    <button onClick={handleSelectAllClasses} className={`text-xs font-black px-3 py-1 rounded-full border ${selectAllClasses ? 'bg-brandPurple text-white border-brandPurple' : 'bg-white text-slate-500 border-slate-200'}`}>
                                        TÜMÜ
                                    </button>
                                </div>
                                <div className="max-h-48 overflow-y-auto p-2">
                                    {regularClasses.map(cls => (
                                        <label key={cls.id} className="flex items-center gap-3 p-2 hover:bg-slate-50 rounded-lg cursor-pointer">
                                            <input type="checkbox" checked={selectedClasses.includes(cls.id) || selectAllClasses} onChange={() => handleClassToggle(cls.id)} className="w-4 h-4 text-brandPurple rounded focus:ring-brandPurple" />
                        ...
                        <button 
                            ...
                            className="w-full mt-4 bg-gradient-to-r from-brandPurple to-blue-500 text-white rounded-xl py-4 font-black tracking-widest text-sm flex items-center justify-center gap-2 shadow-glow hover:opacity-90 transition-opacity"
                        >
