            ...
            >
                <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                    <h3 className="font-bold text-slate-800 flex gap-3 items-center">
                        <div className="bg-purple-100 p-2 rounded-lg"><Library size={20} className="text-primary"/></div> 
                        Kütüphane Yönetimi
                    </h3>
                ...
                <div className="p-4 bg-white">
                    <div className="flex bg-slate-100 p-1.5 rounded-xl">
                        <button onClick={() => setLibraryCategory(LIBRARY_TYPES.TOPIC)} className={`flex-1 py-2 rounded-lg text-xs md:text-sm font-bold transition-all hover-lift ${libraryCategory === LIBRARY_TYPES.TOPIC ? 'bg-white text-primary shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Ödevler</button>
                ...
                <div className="p-5 bg-slate-50 border-b border-slate-100 flex flex-col gap-3">
                    <div className="flex gap-2">
                        <input 
                            type="text" 
                            className="flex-1 p-3 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-800 focus:border-primary outline-none shadow-sm hover-lift" 
                        ...
                        />
                        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => { addLibraryItem(libraryInput); setLibraryInput(''); }} className="bg-primary text-white px-5 rounded-xl hover:bg-purple-700 shadow-glow font-bold">Ekle</motion.button>
                    </div>
