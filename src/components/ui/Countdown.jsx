    ...
    if (minimalist) {
        return (
            <div className="flex flex-col items-center w-full">
                {/* Gelişmiş, Profesyonel Etiket (YKS 2027 vb.) */}
                <div className="relative mb-6 mt-1 group cursor-default">
                    {/* Arka plan parlama efekti */}
                    <div className="absolute inset-0 bg-gradient-to-r from-amber-500/0 via-amber-500/30 to-rose-500/0 rounded-full blur-md opacity-70 group-hover:opacity-100 transition-opacity duration-700"></div>
                    
                    {/* Ana etiket kutusu */}
                    <div className="relative flex items-center justify-center gap-3 px-6 py-2 bg-gradient-to-r from-slate-900/90 via-slate-800/95 to-slate-900/90 border border-slate-700/60 rounded-full shadow-[inset_0_1px_1px_rgba(255,255,255,0.1),0_4px_15px_rgba(0,0,0,0.5)] backdrop-blur-md">
                        {/* Sol animasyonlu nokta */}
                        <div className="flex items-center justify-center">
                            <div className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse shadow-[0_0_8px_rgba(244,63,94,0.8)]"></div>
                        </div>
                        
                        {/* Metin */}
                        <span className="text-xs md:text-sm font-black uppercase tracking-[0.25em] bg-gradient-to-br from-white via-slate-200 to-slate-400 bg-clip-text text-transparent drop-shadow-sm">
                            {targetLabel || 'SINAV'}
                        </span>
                        
                        {/* Sağ animasyonlu nokta */}
                        <div className="flex items-center justify-center">
                            <div className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse shadow-[0_0_8px_rgba(245,158,11,0.8)]" style={{ animationDelay: '0.5s' }}></div>
                        </div>
                    </div>
                </div>

                <div className="flex justify-center gap-4 md:gap-8 w-full">
                    <CircularProgress value={timeLeft.progressDays} max={timeLeft.days} strokeColor="text-rose-400" label="GÜN" small={true} />
                    <CircularProgress value={timeLeft.progressHours} max={timeLeft.hours} strokeColor="text-amber-400" label="SAAT" small={true} />
                    <CircularProgress value={timeLeft.progressMinutes} max={timeLeft.minutes} strokeColor="text-sky-400" label="DK" small={true} />
                </div>
            </div>
        );
    }
