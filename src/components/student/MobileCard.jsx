        ...
        <div className="bg-white rounded-2xl shadow-soft border border-slate-200 overflow-hidden mb-4 transition-all duration-300">
            <div className="p-4 flex items-center justify-between cursor-pointer bg-slate-50/50 hover:bg-slate-50 transition-colors" onClick={() => setExpanded(!expanded)}>
                <div className="flex items-center gap-3">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-lg shadow-sm text-white ${cls.type === 'vip' ? 'bg-gradient-to-br from-amber-400 to-orange-500' : 'bg-primary'}`}>{student.name.charAt(0)}</div>
                                            ...
                                            {topic.subColumns?.map(col => {
                                                ...
                                                return (
                                                    <div key={col.id} className="bg-white p-3 rounded-lg border border-slate-100 shadow-sm">
                                                        {/* 🔥 GÜNCELLEME: onOpenNote tetiğine stopPropagation ve relative z-30 koruması çakıldı */}
                                                        <div className="flex justify-between items-start mb-2"><div className="font-bold text-sm text-slate-700">{col.title}</div><button onClick={(e) => { e.stopPropagation(); onOpenNote(cls.id, student.id, col.id, student.assignmentNotes?.[col.id] || ""); }} className={`p-1.5 rounded-md transition-colors relative z-30 ${hasNote ? 'bg-amber-100 text-amber-600' : 'bg-slate-100 text-slate-400 hover:text-amber-500'}`} title="Öğretmen Notu"><StickyNote size={14} /></button></div>
