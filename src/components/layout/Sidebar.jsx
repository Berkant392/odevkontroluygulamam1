...
import { Home, LayoutDashboard, BookOpenCheck, Compass, MonitorPlay, Target, Layers, Users, Crown, ClipboardList, MessageCircle, BellRing, Library, X, LogOut, Settings, ChevronRight, ChevronLeft, Rocket, Hexagon, AlertTriangle } from 'lucide-react';
...
const Sidebar = ({ 
    ...
    // Mobil çekmece menüsü açıkken scroll'u kilitle
    useEffect(() => {
        if (isMobile && isOpen) {
            lockScroll();
        }
    ...
    const teacherMenu = [
        { id: 'home', label: 'Sınıflarım', icon: Users },
        { id: 'vip-classes', label: 'VIP Dersler', icon: Crown },
        { id: 'reminders', label: 'Hatırlatıcılar', icon: ClipboardList },
        { id: 'subject-study', label: 'Konu Haritası', icon: Hexagon },
        { id: 'whatsapp', label: 'WhatsApp Mesajı', icon: MessageCircle },
        { id: 'send-notification', label: 'Bildirimler', icon: BellRing },
        { id: 'library', label: 'Kütüphane', icon: Library },
        { id: 'bug-reports', label: 'Gelen Hatalar', icon: AlertTriangle },
    ];

    const studentMenu = [
        { id: 'home', label: 'Ana Sayfa', icon: LayoutDashboard },
        { id: 'homework', label: 'Ödevler', icon: BookOpenCheck },
        { id: 'curriculum', label: 'Müfredat', icon: Compass },
        { id: 'playlists', label: 'Videolar', icon: MonitorPlay },
        { id: 'trialTracker', label: 'Denemeler', icon: Target },
        { id: 'subject-study', label: 'Konu Haritası', icon: Hexagon },
        { id: 'questions', label: 'Soru Kartlarım', icon: Layers }
    ];
        ...
        <div className={`h-full flex flex-col justify-between ${currentUserRole === 'vip-student' ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} border-r shadow-xl relative`}>
            ...
            <div className="flex items-center justify-between p-4 h-16 shrink-0 border-b border-opacity-50">
                <div className={`flex items-center gap-3 overflow-hidden ${isExpanded ? 'w-auto' : 'w-0 opacity-0'} transition-all duration-300`}>
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${currentUserRole === 'vip-student' ? 'real-gold-bg' : 'bg-primary'}`}>
            ...
            <div className="flex-1 py-6 flex flex-col gap-2 overflow-y-auto overflow-x-hidden px-3">
                ...
                {menuItems.length === 0 && (
                    <div className="flex flex-col items-center justify-center h-full text-slate-400 opacity-60 space-y-3 mt-10">
                        <Rocket size={40} strokeWidth={1.5} className={currentUserRole === 'vip-student' ? 'text-amber-500 mb-2' : 'text-primary mb-2'} />
                    ...
                    return (
                        <button
                            ...
                            className={`flex items-center gap-3 p-3 rounded-xl transition-all duration-300 relative group
                                ${isActive 
                                    ? (currentUserRole === 'vip-student' ? 'bg-slate-800 text-vipGold' : 'bg-primary/10 text-primary font-bold')
            ...
            <div className={`p-3 border-t border-opacity-50 flex flex-col gap-2 ${currentUserRole === 'vip-student' ? 'border-slate-800' : 'border-slate-100'}`}>
                <button
                    onClick={() => { onReportBug(); if (isMobile) setIsOpen(false); }}
                    className={`flex items-center gap-3 p-3 rounded-xl transition-all duration-300 group
                        ${currentUserRole === 'vip-student' ? 'text-amber-400 hover:bg-slate-800 hover:text-amber-300' : 'text-amber-500 hover:bg-amber-50 hover:text-amber-600'}`}
                >
