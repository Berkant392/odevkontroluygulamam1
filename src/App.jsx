...
import { 
    ...
    Book, Star, ChevronLeft, ChevronRight 
} from 'lucide-react';
...
const generateUsername = (name) => {
    if (!name) return `ogrenci.${Math.floor(100+Math.random()*900)}`;
...
const App = () => {
    ...
    // --- DURUM GÜNCELLEME (PENCEREDEN) ---
    const handleUpdateGrade = async (classId, studentId, colId, newStatus) => {
        const cls = classes.find(c => c.id === classId);
        if (!cls) return;
        const studentIdx = cls.students.findIndex(s => s && s.id === studentId); // BOZUK VERİ KORUMASI
    ...
    const addToLibrary = async (text, type) => {
        ...
        if (!exists) await setDoc(doc(collection(db, 'berkant_hoca_library')), { text, type });
    };
    ...
    const handleModalSubmit = async (inputVal) => {
        ...
        if (modal.type === 'class') {
            ...
            // BOZUK VERİ KORUMASI: cls.students.filter(Boolean)
    ...
    // --- ŞİFRE GÜNCELLEME (ÖĞRENCİ AYARLARI) ---
    ...
    const handleStudentLogin = () => {
        ...
        if (found) { setCurrentUserRole('student'); setLoggedInStudent(found.student); setSelectedClass(found.class); setAuthView('selection'); setStudentUser(""); setStudentPass(""); }
                        ...
                        {authView === 'selection' ? (
                            <div className="space-y-4">
                                <button onClick={() => setAuthView('student')} className="w-full p-6 bg-white/5 border border-white/10 rounded-2xl flex items-center gap-5 transition-all text-white hover:bg-white/10">
                                    <User className="text-indigo-400" size={24}/><p className="font-bold text-lg text-left flex-1">Öğrenci & Veli</p><ChevronRight className="text-slate-500" />
                                </button>
                                <button onClick={() => setAuthView('teacher')} className="w-full p-6 bg-white/5 border border-white/10 rounded-2xl flex items-center gap-5 transition-all text-white hover:bg-white/10">
                                    <ShieldAlert className="text-slate-400" size={24}/><p className="font-bold text-lg text-left flex-1">Öğretmen</p><ChevronRight className="text-slate-500" />
                                </button>
                            </div>
                        ) : (
                            <div className="space-y-6">
                                ...
                                {authView === 'student' ? (
                                    <div className="space-y-4">
                                        ...
                                        <button onClick={handleStudentLogin} className="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black shadow-xl hover:bg-indigo-500 transition-all">SİSTEME GİRİŞ YAP</button>
                                    </div>
                                ) : (
                                    <div className="space-y-4">
                                        ...
                                        <button onClick={verifyTeacherPin} className="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black hover:bg-indigo-500 transition-all">YÖNETİMİ AÇ</button>
                                    </div>
                                )}
                            </div>
                        )}
    ...
    return (
        <div className="min-h-screen bg-[#f8fafc]">
            {/* Header'a Ayarlar Fonksiyonu Eklendi */}
            <Header 
                role={currentUserRole} 
                onLogout={() => setCurrentUserRole(null)} 
                dailyQuote={dailyQuote} 
                onOpenSettings={() => setStudentSettingsModal(true)} 
            />
                ...
                {currentUserRole === 'teacher' ? (
                    <AdminPanel 
                        ...
                        onAddStudent={async (clsId, name) => {
                            ...
                            const nStd = { id: generateId('std'), name: name.trim(), username: generateUsername(name.trim()), password: generatePassword(), grades: {} };
                        ...
                        calculateStats={(s, t) => {
                            ...
                            // BOZUK VERİ KORUMASI: s.filter(Boolean)
                ...
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
                    <div className="bg-white rounded-[2rem] w-full max-w-xs shadow-2xl overflow-hidden animate-fadeIn">
                        ...
                        <div className="p-4 grid grid-cols-1 gap-2">
                            ...
                            <button onClick={() => setStatusModal(null)} className="w-full py-3 text-slate-400 font-bold text-xs mt-2 italic hover:text-slate-600">İptal Et</button>
                        </div>
                    </div>
                </div>
            )}

            {/* --- KÜTÜPHANELİ EKLEME MODALI --- */}
            {modal.type && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
                    <div className="bg-white rounded-[2.5rem] w-full max-w-md shadow-2xl overflow-hidden">
                        <div className="p-6 bg-slate-50 border-b flex justify-between items-center font-black text-slate-800 uppercase tracking-tighter">
                            {modal.type === 'class' ? 'Sınıf Ekle' : (modal.type === 'topic' ? 'Ödev Ekle' : 'Kaynak Ekle')}
                        ...
                        <div className="p-8 space-y-6">
                            <div>
                                ...
                                <input id="modalInput" type="text" autoFocus className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:border-indigo-500 font-bold" placeholder="..." />
                            </div>
                            
                            {modal.type !== 'class' && (
                                <div>
                                    <label className="text-[10px] font-bold text-indigo-400 uppercase mb-2 flex items-center gap-1 tracking-widest"><Book size={12}/> Kütüphaneden Seç</label>
                                    <div className="max-h-40 overflow-y-auto border border-slate-100 rounded-2xl p-2 bg-slate-50/50 grid grid-cols-1 gap-1">
                                        {library.filter(i => i.type === modal.type).length > 0 ? (
                                            library.filter(i => i.type === modal.type).map(item => (
                                                <button key={item.id} onClick={() => { document.getElementById('modalInput').value = item.text; }} className="text-left p-3 rounded-xl hover:bg-white hover:shadow-sm text-xs font-bold text-slate-600 flex items-center gap-2"><Star size={10} className="text-amber-400 shrink-0"/> {item.text}</button>
                                            ))
                                        ) : <div className="text-[10px] text-slate-400 p-4 italic text-center">Kütüphane boş...</div>}
                                    </div>
                                </div>
                            )}

                            <button onClick={() => handleModalSubmit(document.getElementById('modalInput').value)} className="w-full py-5 bg-indigo-600 text-white rounded-[1.5rem] font-black shadow-xl hover:bg-indigo-500 transition-all">SİSTEME EKLE</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {studentSettingsModal && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
                    <div className="bg-white rounded-[2.5rem] w-full max-w-sm shadow-2xl overflow-hidden">
                        <div className="p-6 bg-slate-50 border-b flex justify-between items-center font-black text-slate-800 uppercase tracking-tighter">Şifre Güncelle <button onClick={() => setStudentSettingsModal(false)} className="text-slate-400"><X/></button></div>
                        <div className="p-8">
                            ...
                            <button onClick={updateStudentPassword} className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black">GÜNCELLE</button>
                        </div>
                    </div>
                </div>
            )}

            {/* --- YAZDIRMA (ŞİFRE VE ÖZEL KARNE) --- */}
            {printData && (
